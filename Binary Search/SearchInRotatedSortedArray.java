...
class SearchInRotatedSortedArray {
    public int search(int[] nums, int target) {
        ...
        while(low <= high)
        {
            ...
            // Condition to check wheather left is sorted or not.
            if(nums[low] <= nums[mid])
            {
                if(nums[low] <= target && target < nums[mid])  // Checking if target is present in sorted left
                {
                    high = mid -1;
                }else {
                    low = mid + 1;
                }
            }else{  // If left is not sorted then right sorted
                if(nums[mid] < target && target <= nums[high]) // Check if target is present in sorted right
                {
                    low = mid + 1;
                }else {
                    high = mid - 1;
